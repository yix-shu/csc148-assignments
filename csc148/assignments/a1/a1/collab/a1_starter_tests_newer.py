...
from scheduler import GreedyScheduler, _smaller_volume, _smaller_city, \
    _less_truck_space, _more_truck_space, _capable_trucks
...
test_arguments = [
    ...
    ('2-self',
     {
         'depot_location': 'Toronto',
         'parcel_file': 'data/demo-parcel-data.txt',
         'truck_file': 'data/demo-truck-data.txt',
         'map_file': 'data/map-data.txt',
         'algorithm': 'greedy',
         'parcel_priority': 'volume',
         'parcel_order': 'non-decreasing',
...
# def test_total_distance_travelled_different_ends() -> None:
#     """Testing when the end location of one parcel and the beginning location
#     of another are not the same city."""
#     f = Fleet()
#     t1 = Truck(1, 10, 'Toronto')
#     f.add_truck(t1)
#     p1 = Parcel(1, 5, 'Hamilton', 'Vancouver')
#     assert t1.pack(p1) is True
#     m = DistanceMap()
#     m.add_distance('Toronto', 'Hamilton', 2)
#     m.add_distance('Hamilton', 'Vancouver', 3)
#     m.add_distance('Vancouver', 'Toronto', 6)
#     assert f.total_distance_travelled(m) == 11
...
def _volume_non_decreasing(p1: Parcel, p2: Parcel) -> bool:
    """Return whether parcel <p1> has less volume than parcel <p2>.
    """
    return p1.volume < p2.volume  # <= ?


def _volume_non_increasing(p1: Parcel, p2: Parcel) -> bool:
    """Return whether parcel <p1> has more volume than parcel <p2>.
    """
    return p1.volume > p2.volume  # >= ?


def _destination_non_decreasing(p1: Parcel, p2: Parcel) -> bool:
    """Return whether parcel <p1>'s destination comes before parcel <p2>'s
    destination.
    """
    return p1.destination < p2.destination


def _destination_non_increasing(p1: Parcel, p2: Parcel) -> bool:
    """Return whether parcel <p1>'s destination comes after parcel <p2>'s
    destination.
    """
    return p1.destination > p2.destination


def _eligible_trucks(parcel: Parcel, trucks: List[Truck]) -> List[Truck]:
    """Returns a list of eligible trucks for the given parcel. A truck is
    eligible if it has enough unused volume to store parcel, and
    if there are trucks with the parcel's destination at the end of their route,
    they have precedence.
    """
    eligible_trucks = []
    for truck in trucks:
        if truck.route[-1] == parcel.destination and \
                truck.volume - truck.used_space >= parcel.volume:
            eligible_trucks.append(truck)
    if len(eligible_trucks) == 0:
        for truck in trucks:
            if truck.volume - truck.used_space >= parcel.volume:
                eligible_trucks.append(truck)
    return eligible_trucks


def _pick_truck(eligible_trucks: List[Truck],
                _truck_order: str) -> Union[Truck, None]:
    """Return the truck that fits the given <_truck_order> for the given parcel.
    """
    if _truck_order == 'non-decreasing':
        return _trucks_non_decreasing(eligible_trucks)
    elif _truck_order == 'non-increasing':
        return _trucks_non_increasing(eligible_trucks)
    return None


def _trucks_non_increasing(trucks: List[Truck]) -> Truck:
    """Return the Truck with the most space available.
    """
    max_size = None
    chosen_truck = None
    ...
    for truck in trucks:
        if chosen_truck is None:
            max_size = truck.volume - truck.used_space
            chosen_truck = truck
        else:
            if truck.volume - truck.used_space > max_size:
                max_size = truck.volume - truck.used_space
                chosen_truck = truck
    return chosen_truck


def _trucks_non_decreasing(trucks: List[Truck]) -> Truck:
    """Return the Truck with the least available space.
    """
    min_size = None
    chosen_truck = None
    ...
    for truck in trucks:
        if min_size is None:
            min_size = truck.volume - truck.used_space
            chosen_truck = truck
        else:
            if truck.volume - truck.used_space < min_size:
                min_size = truck.volume - truck.used_space
                chosen_truck = truck
    return chosen_truck
...
def test_priority_truck_non_increasing_destination() -> None:
    """Test the doctest provided for PriorityQueue.add and
    PriorityQueue.remove"""
    t1 = Truck(1, 25, 'York')
    t2 = Truck(2, 10, 'York')
    ...
    et = _capable_trucks(p1, trucks)
    assert len(et) == 5
    pq = PriorityQueue(_more_truck_space)
    print(et)
    for t in et:
        pq.add(trucks[t])
    truck = pq.remove()
...
if __name__ == '__main__':
    pytest.main(['a1_starter_tests_newer.py'])
