...
class ExprTree:
    ...
    def eval(self, lookup: Dict[str, int]) -> int:
        ...
        """
        if self.is_empty():
            # Optional case. Won't be tested.
            return 0
        elif not self._subtrees:
            # Since subtrees is empty, root cannot be an operator.
            if isinstance(self._root, int):
                return self._root
            else:  # if isinstance(self._root, str):
                # Use lookup to return corresponding value
                return lookup[self._root]
        else:
            # Since subtrees is not empty, then root must be an operator.
            if self._root == OP_ADD:
                summ = 0  # I swear I spelled it like this on purpose.
                for subtree in self._subtrees:
                    # Recursively call eval and sum up the evaluated subtrees.
                    summ += subtree.eval(lookup)
                return summ
            else:  # If self._root == '*':
                product = 1
                for subtree in self._subtrees:
                    # Recursively call eval and multiply the evaluated subtrees.
                    product *= subtree.eval(lookup)
                return product
    ...
    def __str__(self) -> str:
        ...
        if self.is_empty():
            ...
            # Since subtrees is not empty, then root must be an operator.
            if self._root == OP_ADD:
                # Create a string as the open bracket and the first subtree
                sum_str = f'({self._subtrees[0]}'
                # Loop through the subtree list from index 1 onwards.
                for subtree in self._subtrees[1:]:
                    # Recursively call __str__ on each subtree. Include space
                    # around operator to match required format.
                    sum_str += ' + ' + str(subtree)
                # Close string, and return.
                sum_str += ')'
                return sum_str
            else:  # if self._root == '*':
                # Same steps as above, except with * operator.
                product_str = f'({self._subtrees[0]}'
                for subtree in self._subtrees[1:]:
                    product_str += ' * ' + str(subtree)
                product_str += ')'
                return product_str
    ...
    def substitute(self, from_to: Dict[Union[str, int],
        ...
        >>> print(exp_t)
        (2 + (2 + 1))
        >>> exp_t = construct_from_list([['+'], ['a', 'b', '+'], ['a', 'b']])
        >>> exp_t.substitute({'a': 1, 'b': 2})
        >>> print(exp_t)
        (1 + 2 + (1 + 2))
        """
        if self.is_empty():
            # If ExpTree is empty, do nothing.
            return None
        else:
            # Check if the root of the tree is in the dictionary.
            if self._root in from_to:
                # If it is, mutate the root node to its corresponding dic value.
                if 0 != from_to[self._root]:
                    self._root = from_to[self._root]
            # Recursively call substitute on each of the subtrees.
            for subtree in self._subtrees:
    ...
    def populate_lookup(self, lookup: Dict[str, int]) -> None:
        ...
        if self.is_empty():
            ...
            # Check whether the root is a variable, and whether it is in lookup.
            if isinstance(self._root, str) and (self._root not in OPERATORS):
                lookup[self._root] = 0
            # Call populate_lookup on each subtree recursively.
            for subtree in self._subtrees:
