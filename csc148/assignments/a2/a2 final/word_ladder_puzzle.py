...
class WordLadderPuzzle(Puzzle):
    ...
    def __eq__(self, other: WordLadderPuzzle) -> bool:
        ...
        True
        """
        # Create booleans storing equality data
        from_check = self.from_word == other.from_word
        to_check = self.to_word == other.to_word
        word_set_check = self.word_set == other.word_set
        # Check if they are all true.
        return from_check and to_check and word_set_check
    ...
    def extensions(self) -> List[WordLadderPuzzle]:
        ...
        True
        """
        # Initialise the return list. Create a list containing the characters of
        # <from_word> to make it easier to work with.
        lst = []
        from_word_chars = list(self.from_word)
        # Iterate through the indices of <original_word>, and then
        # iterate through the characters (i.e. the alphabet)
        for i in range(len(from_word_chars)):
            for char in self._chars:
                # Create a copy of <from_word_chars> so we don't mutate it.
                potential_word_chars = from_word_chars[:]
                # Set the i-th character to be <char>.
                potential_word_chars[i] = char
                # Make the word from the list.
                potential_word = ''.join(potential_word_chars)
                # Check if it is in the set, and that the word is not
                # the <from_word> already.
                if potential_word in self.word_set \
                        and potential_word != self.from_word:
                    # If it is, then create a new WordLadderPuzzle and append.
                    potential = WordLadderPuzzle(potential_word, self.to_word,
                                                 self.word_set)
    ...
    def get_difficulty(self) -> str:
        ...
        """
        # First check if the puzzle is already solved.
        if self.is_solved():
            return TRIVIAL
        # Now initialise the BfsSolver (since its faster than DfsSolver) and
        # find how many steps the solution takes.
        bfs = BfsSolver()
        path_length = len(bfs.solve(self)) - 1
        # Check conditions according to the docstring.
        if path_length == -1:  # When bfs.solve(self) == []
            return IMPOSSIBLE
        if path_length <= 1:
            return TRIVIAL
        elif path_length == 2:
