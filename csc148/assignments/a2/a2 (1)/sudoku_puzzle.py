...
class SudokuPuzzle(Puzzle):
    ...
    def fail_fast(self) -> bool:
        ...
        True
        """
        if (not self.extensions()) and (not self.is_solved()):
            return True
        # temporary variables to give convenient names to each attribute
        r = 0  # row with first empty position
        while r < self._n:
            c = 0
            for item in self._grid[r]:
                if not self._check_empty_allowed(item, r, c):
                    return True
                c += 1
            r += 1
        return False

    def _check_empty_allowed(self, cell: str, r: int, c: int) -> bool:
        """
        Checks whether the cell is empty. If it is, then:

        Returns True if the empty cell has allowed symbols or the cell is not
        empty.
        Returns False if the empty cell does not have allowed symbols.
    ...
    def has_unique_solution(self) -> bool:
        ...
        bfs = BfsSolver()
        solvers = [dfs, bfs]
        solutions = {dfs.solve(self)[-1].__str__(),
                     bfs.solve(self)[-1].__str__()}
        temp = 0
        while temp != len(solutions):
            temp = len(solutions)
            for s in solvers:
                if s.solve(self, solutions) is not None \
                        and s.solve(self, solutions) != []:
                    solutions.add(s.solve(self, solutions)[-1].__str__())
            if len(solutions) > 1:
                return False
        return True
        """
        dfs = DfsSolver()
        path1 = dfs.solve(self)

        if path1 == []:
            return False

        path2 = dfs.solve(self, {path1[-1].__str__()})
        solution1 = path1[-1]
        if path2 == []:
            return True
        solution2 = path2[-1]

        if solution2 != solution1 and solution2.is_solved():
            return False
        else:
            return True
